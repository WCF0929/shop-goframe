...
type PositionReq struct {
	...
	Link      string `json:"link"    v:"required#跳转连接不能为空" dc:"跳转链接"`
	GoodsName string `json:"goods_name"    v:"required#商品名称不能为空" dc:"商品名称"` //冗余设计
	GoodsId   uint   `json:"goods_id"    v:"required#商品id不能为空" dc:"商品Id"`   //第三范式
	Sort      int    `json:"sort"    dc:"排序"`
}
type PositionRes struct {
	//g.Meta     `mine:"text/html" example:"string"`
	PositionId uint `json:"position_id"`
}

type PositionDeleteReq struct {
	g.Meta `path:"/position/delete"   method:"delete"  tags:"手工位图"summary:"删除内容接口"`
	Id     uint `v:"min:1#请选择要删除的手工位图" dc:"手工位图id"`
}
type PositionDeleteRes struct {
}

type PositionUpdateReq struct {
	g.Meta    `path:"/position/update/{Id}" method:"post" tags:"手工位图" summary:"修改内容接口"`
	Id        uint   `json:"id"      v:"min:1#请选择需要修改的内容" dc:"内容Id"`
	PicUrl    string `json:"pic_url"    v:"required#手工位图链接不能为空" dc:"图片链接"`

...
type RotationDeleteReq struct {
	g.Meta `path:"/delete"   method:"delete"  tags:"轮播图"summary:"删除内容接口"`
	Id     uint `v:"min:1#请选择要删除的轮播图" dc:"轮播图id"`
}
type RotationDeleteRes struct {
}

type RotationUpdateReq struct {
	g.Meta `path:"/rotation/update/{Id}" method:"post" tags:"轮播图" summary:"修改内容接口"`
	Id     uint   `json:"id"      v:"min:1#请选择需要修改的内容" dc:"内容Id"`
	PicUrl string `json:"pic_url"    v:"required#轮播图链接不能为空" dc:"图片链接"`
